...
use aws_sdk_s3::{
    config, error::SdkError, operation::get_object::GetObjectError, primitives::ByteStream,
...
pub async fn client(
    ...
    let config = if let Some(creds) = credentials {
    ...
    let client = aws_sdk_s3::Client::new(&config.load().await);
